...
def encryptAudio(obj, key, chunkSize=5000):
	# Grabbing all the frames from the wave object
	...
	# Forming the encrypted bytearray
	key = sorted(key)
	newFrames = bytearray()
	for colNumber in key:
		newFrames.extend(matrix[colNumber])
	
	# Storing the encrypted bytearray as a .wav file and setting required parameters
...
def decryptAudio(obj, key):
	...
	# Forming the decrypted bytearray
	newFrames = bytearray()
	for i in range(0, colLength):
		for colNumber in key:
			newFrames.append(matrix[colNumber][i]) 

	# Storing the decrypted bytearray as a .wav file
